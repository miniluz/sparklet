...
#let todo(inline: false, label: none, body) = {
  counter("todo").step()
  ...
  let capt = to-string(body)
  capt = capt.slice(0, calc.min(40, capt.len()))
  show figure.where(kind: "todo"): it => {}
  box(width: 0pt, height: 0pt, [
    #figure(kind: "todo", supplement: "TODO", gap: 0pt, caption: capt, none)
    #if label != none {
      label
    }])
  if inline {
...
#let TFC(
  ...
  color-principal: rgb("#891536"),
  color-secundario: rgb("#f6e3a8"),
  incluye-toc-figuras: true,
  incluye-toc-tablas: true,
  incluye-toc-codigos: true,
  incluye-toc-todo: false,
  ...
  #set text(
    lang: if secciones-ingles { "en" } else { "es" },
    size: 12pt,
    font: font,
    hyphenate: false,
  )
  ...
  #set math.equation(numbering: it => numbering(
    "(1.1)",
    counter(heading).get().at(0),
    it,
  ))
  #show figure.where(kind: image): set figure(numbering: it => numbering(
    "1.1",
    counter(heading).get().at(0),
    it,
  ))
  #show figure.where(kind: table): set figure(numbering: it => numbering(
    "1.1",
    counter(heading).get().at(0),
    it,
  ))
  #show figure.where(kind: "todo"): set figure(numbering: it => numbering(
    "1.1",
    counter(heading).get().at(0),
    it,
  ))
  #show figure.where(kind: raw): set figure(
    supplement: if secciones-ingles { "Code" } else { "Código" },
    numbering: it => numbering("1.1", counter(heading).get().at(0), it),
  )
  ...
  #[
    ...
    #v(2cm)
    #block(
      fill: color-secundario.lighten(50%),
      width: 100%,
      inset: (y: 1cm),
      ...
      #[#set text(size: 14pt)
          ...
          if portada-ingles { "FINAL DEGREE PROJECT" } else {
      ...
      *#text(size: 13pt, [#sym.diamond.filled] * 3)*
  ...
  #show heading.where(level: 1): it => {
    ...
    counter(figure.where(kind: "todo")).update(0)
  ...
  #show heading.where(level: 6): it => {
    text(
      fill: red,
      [*QUEDA TERMINANTEMENTE PROHIBIDO USAR ESTE NIVEL DE ENCABEZADO*],
    )
  }
  ...
  // Bibliografía
  #show cite: it => {
    show regex("\d+"): set text(fill: color-principal)
    it
  }
  ...
  // Estilo de referencias. Incluyendo un enlace en el caso de las figuras y ecuaciones

  #show ref: it => {
    let el = it.element
    // Figuras, equaciones y encabezados
    if el != none and el.func() in (figure, math.equation, heading) {
      if (el.func() == heading and el.level == 1) {
        let num = counter(heading).at(el.location()).at(0)
        link(el.location(), [#if (secciones-ingles) { "Chapter" } else {
            "Capítulo"
          } #num])
      } else {
        link(el.location())[#it]
      }
      // Lo demás (bibliografía)
    } else {
      it
    }
  }
  ...
  *Keywords:* #keywords.join(", ")
  ...
  #outline(title: if secciones-ingles { "Table of contents" } else {
    "Índice general"
  })

  // Estilos para outlines secundarias
  // Hacer que no aparezca el primer nivel resaltado
  #show outline.entry.where(level: 1): it => {
  ...
  #if (incluye-toc-codigos) {
    outline(
      title: if secciones-ingles { "List of code extracts" } else {
  ...
  #if (incluye-toc-figuras) {
    outline(
      title: if secciones-ingles { "List of figures" } else {
        "Índice de figuras"
      },
      target: figure.where(kind: image),
    )
  }
  #if (incluye-toc-tablas) {
    outline(
      title: if secciones-ingles { "List of tables" } else {
        "Índice de tablas"
      },
  ...
  #if (incluye-toc-todo) {
    outline(
      title: if secciones-ingles { "List of TODOs" } else { "Índice de TODOs" },
      target: figure.where(kind: "todo"),
    )
  }

  // Cambio ahora para que la bibliografía no sea de colores
  ...
  #show heading.where(level: 1): it => {
    ...
    v(2cm)
    block(width: 100%, inset: 1cm, stroke: (
      top: 3pt + color-principal,
      bottom: 3pt + color-principal,
    ))[

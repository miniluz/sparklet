...
casi inmediata, por lo que el bloque tiene que haber sido calculado de antemano. Debido a esto, la generación de los
bloques de audio se realiza en otra tarea: el motor de síntesis. Son conectadas con una cola de 2 bloques de audio
completos con _back-pressure_, lo que implica que la cola bloquea al escritor (el motor de síntesis) hasta que hay un
espacio libre (se envía un bloque de audio). Una vez enviado el bloque, se marca como consumido, lo que despierta el
motor de síntesis para que genere el siguiente.
...
periféricos y la configuración de su hardware relevante en un formato común. El módulo `hardware` exporta únicamente la
